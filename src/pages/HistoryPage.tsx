        ...
        <section className="relative h-[65vh] md:h-[60vh] overflow-hidden group">
          ...
          <div className="absolute inset-0 flex items-center justify-center text-center px-6 pt-10">
            ...
            >
              ...
              <h1 className="text-5xl sm:text-6xl md:text-8xl font-sans font-black text-white tracking-tighter mb-4 italic leading-tight">
                {data.school_history?.[0]?.title || (
                  <>The Legacy <br /> <span className="text-school-neon tracking-tighter text-3xl sm:text-4xl md:text-5xl uppercase font-black not-italic opacity-90">Archive.</span></>
                )}
              </h1>
        ...
        <section className="py-20 md:py-40 bg-school-paper relative z-10 -mt-12 md:-mt-20 rounded-t-[40px] md:rounded-t-[100px] shadow-2xl border-t border-school-ink/10">
          <div className="max-w-4xl mx-auto px-6 lg:px-12">
            <div className="space-y-16 text-xl text-slate-800 leading-relaxed font-medium text-justify">
              {data.school_history?.[0]?.content ? (
                <motion.div 
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true }}
                  className="whitespace-pre-wrap text-justify prose prose-slate max-w-none shadow-none bg-transparent prose-p:my-4 prose-headings:text-school-navy"
                  dangerouslySetInnerHTML={{ __html: data.school_history[0].content }}
                />
              ) : (
                <>
                  <motion.p 
