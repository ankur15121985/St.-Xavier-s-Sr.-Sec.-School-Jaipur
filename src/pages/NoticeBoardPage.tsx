  ...
  return (
    <Layout data={data}>
      ...
      <div className="bg-school-paper min-h-screen">
          ...
          <div className="space-y-6">
            {notices.map((notice, idx) => (
              <motion.div
                ...
                className="group relative bg-school-paper rounded-[32px] p-8 md:p-12 border border-school-ink/10 shadow-sm hover:shadow-2xl hover:border-school-gold/30 transition-all duration-500"
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                  <div className="flex-1">
                    <div className="flex items-center gap-4 mb-6">
                      <div className="flex items-center gap-2 px-4 py-1.5 bg-school-paper/50 rounded-full border border-school-ink/5">
                        <Calendar size={12} className="text-school-ink/40" />
                        <span className="text-[10px] font-black uppercase tracking-widest text-school-ink/60">{notice.date}</span>
                      </div>
                  ...
                  <div className="flex items-center gap-4">
                    {(notice.attachmentUrl || notice.link) && (
                      <a 
                        ...
                        className="flex items-center gap-3 px-8 py-4 bg-school-ink text-school-paper rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-school-accent hover:text-white transition-all shadow-xl active:scale-95"
                      >
                        <Download size={14} className="text-school-gold" />
                        View Attachment
                      </a>
                    )}
                    <div className="w-12 h-12 rounded-2xl bg-school-paper/50 flex items-center justify-center text-school-ink opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all">
        ...
        {/* Contact/Help Link */}
        <section className="py-24 bg-school-paper border-t border-school-ink/10">
          <div className="max-w-7xl mx-auto px-6 text-center">
            <div className="inline-block p-12 rounded-[48px] bg-school-paper/50 border-2 border-dashed border-school-ink/20">
              <Clock className="mx-auto text-school-ink mb-6" size={48} />
              <h4 className="text-2xl font-serif font-black text-school-ink italic uppercase mb-4">Frequency of Updates</h4>
              <p className="text-school-ink/60 text-sm max-w-xl mx-auto leading-relaxed font-medium">
